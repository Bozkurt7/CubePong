...
yazi.penup()
yazi.goto(0, 380)
yazi.write("Oyuncu A:0   Oyuncu B:0", align='center', font=('Courier', 35, 'bold'))
yazi.hideturtle()
puan_a = 0
puan_b = 0
...
def restart_game(x, y):
      ...
      turtle.listen()
      turtle.onkeypress(lambda: restart_game(-1, -1), "E")
      restart_game()
      turtle.done()
...
while True:
    ...
    if ball.xcor()>1550:
        ...
        puan_a = puan_a + 1
        yazi.clear()
        yazi.write("Oyuncu A:{}   Oyuncu B:{}".format(puan_a, puan_b), align='center', font=('Courier', 35, 'bold'))
    if ball.xcor()<-950:
        winsound.PlaySound('f', winsound.SND_ASYNC)
        ball.goto(0, 0)
        ball.dx = ball.dx * -1
